            ...
            if (roObject != null)
            {
                if (roObject.IsValid(roContext, roMess))
                {
                    ...
                    if (roObject.ID == 0)   // New object
                    {
                        ...
                        if (oFCObj is null) throw new Exception($"Can't get new {roObject.GetType().Name}");
                    }
                    else                    // Saving existing object
                    {
                        oFCObj = PrGet<tObject>(roContext, roObject.ID, ACCESSFLAGS.OM_WRITE);      // IsValid already checked for existance
                        if (oFCObj is null) throw new Exception($"Can't open {roObject.GetType().Name} for writing");
                    }
                    ...
                    try { oFCObj.WriteUnlock(); }
                    catch (COMException e) 
                    {
                        roMess.Add("RF0313", e.Message, roObject.Name, roObject.GetType().Name);
                        _oLog.Debug($"Error {e} in releasing {roObject.Name}");
                    }
                    _oLog.Debug("Writen");
